...
# GPS Simulator and API Adapter Launcher
# This script starts both the GPS simulator and API adapter services
...
# Print banner
echo "================================="
echo "  GPS Simulator System Launcher  "
echo "================================="
echo "Docker host IP: $DOCKER_HOST_IP"
echo "Database connection: 127.0.0.1:3306"

# Start API adapter in background
...
# Start GPS simulator in foreground
echo "[+] Starting GPS Simulator on port $SIMULATOR_PORT..."
echo "[+] Using Docker-compatible settings for API connectivity"
echo "[+] Access the web interface at: http://localhost:$SIMULATOR_PORT"
echo "[+] Press Ctrl+C to stop both services"

# Run the simulator with improved settings
python3 "$SIMULATOR" \
    --port $SIMULATOR_PORT \
